...
class MainWindow(QMainWindow):
    ...
    def show_new_window(self, checked) -> None:
        ...
        if self.w is None:  # проверяем существование дополнительного окна
            ...
            self.w.close()  # метод закрытия окна, работает даже если не удалять ссылку
            self.w = None  # Обеспечивает закрытие окна путем сброса ссылка на него при повторном
            # нажатии на кнопку вызова окна. Окно удаляется сборщиком мусора питон после удаления ссылки на него.
            # Если ссылка на данное окно храниться еще где-либо, то таким образом окно закрыть не удастся.
            # При этом, если ссылку не удалить, то повторно окно не создастся до перезапуска модуля,
            # т.к. не будет выполняться проверка условия выше.
