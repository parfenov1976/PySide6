...
Также добавим парсеры для предварительной обработки сырых данных.
"""
import sys
import requests
import re
...
"""
Модуль re для работы в регулярными выражениями.
...
class Worker(QRunnable):
    ...
    def __init__(self, id: int, url: str, parsers: dict) -> None:
        """
        ...
        self.parsers = parsers  # сохранение парсеров в аттрибуте рабочего потока
    ...
    def run(self) -> None:
        ...
        data = {}  # создание пустого словаря для сохранения обработанных данных
        for name, parser in self.parsers.items():  # извлечение парсера из словаря в цикле по словарю с парсерами
            m = parser.search(r.text)  # обработка парсером ответа от удаленного сервера
            # (поиск первого совпадения с регулярным выражением)
            if m:  # проверка обработанного ответа на наличие содержимого
                data[name] = m.group(1).strip()  # добавление возвращенной первой подгруппы совпадений с регулярным
                # выражением с очисткой результата от пробельных символов вначале и в конце
        self.signals.data.emit((self.id, data))  # передача кортежа данных объекту сигналов рабочего потока
...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.parsers = {  # парсеры на базе регулярных выражений
    ...
    def execute(self) -> None:
        ...
        for n, url in enumerate(self.urls):  # цикл создания и запуска рабочих потоков
            worker = Worker(n, url, self.parsers)  # создание экземпляра рабочего потока
