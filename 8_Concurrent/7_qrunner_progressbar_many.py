...
и отображение его на одном индикаторе прогресса.
"""
import sys
import time
import random
...
class MainWindow(QMainWindow):
    ...
    def cleanup(self, job_id: uuid) -> None:
        ...
        :return: None
        """
        if job_id in self.worker_progress:  # проверка наличия идентификатора в словаре рабочих потоков
            del self.worker_progress[job_id]  # удаление рабочего потока из словаря
            self.refresh_progress()  # вызов метода на обновление индикатора прогресса
