...
В данном файле рассмотрен пример остановки рабочего потока по требованию (до завершения)
"""
import sys
import time
    ...
    def run(self) -> None:
        ...
        try:  # Начало блока перехвата исключения
            for n in range(100):  # запуск цикла определения степени выполнения
                ...
                time.sleep(0.05)  # установка задержки для эмуляции нагрузки
                if self.is_killed:  # проверка флага завершения рабочего потока
                    raise WorkerKilledException  # возбуждение исключение завершения рабочего потока
        except WorkerKilledException:  # перехват исключения завершения рабочего потока
            pass  # продолжение работы программы без остановки основного цикла событий
