...
Пример рисования прямоугольников с использованием настроек пера (размер и цвет). В данном примере
прямоугольники закрашиваются с помощью кисти для которой задается цвет стиль (текстура).
"""
import sys
...
в библиотеке Qt, класса прямоугольник QRect, 
...
class MainWindow(QMainWindow):
    ...
    def draw_something(self) -> None:
        ...
        pen.setColor(QColor('#EB5160'))  # установка цвета пера
        painter.setPen(pen)  # применение настроек пера к рисовальщику
        brush = QBrush()  # создание экземпляра класса кисти
        brush.setColor(QColor('#FFD141'))  # установка цвета для закрашивания
        brush.setStyle(Qt.BrushStyle.Dense1Pattern)  # установка стиля для закрашивания. Если не поставить стиль,
        # то, закрашиваться ничего не будет, т.к. по умолчанию стоит стиль NoBrush
        # другие стили можно найти в документации к PySide6
        painter.setBrush(brush)  # применение настроек кисти для закрашивания к рисовальщику
        # за раз может быть активным только одна кисть (перо), но их можно переключать во время рисования
        painter.drawRects(
            [
                QRect(50, 50, 100, 100),
                QRect(60, 60, 150, 100),
                QRect(70, 70, 100, 150),  # рисование множества прямоугольников
                QRect(80, 80, 150, 100),  # прямоугольники можно подавать в виде кортежа или списка
                QRect(90, 90, 100, 150)
            ]
        )
