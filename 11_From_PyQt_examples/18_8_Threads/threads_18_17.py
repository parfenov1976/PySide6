"""
Пример блокировщика доступа ресурса при его использовании одним потоком для ресурсов,
не поддерживающих одновременный доступ
"""
...
class MyThread(QThread):
    ...
    def change_x(self) -> None:
        ...
        :return: None
        """
        MyThread.mutex.lock()  # блокировка доступа к ресурсу для остальных потоков
        ...
        print(f'x ={MyThread.x} id ={self.id}')  # вывод состояния ресурса и id потока, который имеет к нему доступ
        MyThread.mutex.unlock()  # снятие блокировки ресурса после обработки текущим потоком
