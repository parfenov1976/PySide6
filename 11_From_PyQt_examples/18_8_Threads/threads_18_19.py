...
не поддерживающих одновременный доступ. В данном примере использован автоблокировщик совместно
с обработчиком контекстов with
"""
...
class MyThread(QThread):
    ...
    def change_x(self) -> None:
        ...
        :return: None
        """
        with QMutexLocker(MyThread.mutex):  # создаем экземпляр автоблокировщика и тем самым накладываем блокировку
            ...
            print(f'x ={MyThread.x} id ={self.id}')  # вывод состояния ресурса и id потока, который имеет к нему доступ
            # по достижении конца кода в рамках обработчика контекста блокировка будет снята
